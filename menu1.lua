...
local function draw_window(win)
    ...
    valex.draw_filled_rect(win.x, win.y, win.w, win.h, menu.theme.background)
    valex.draw_rect(win.x, win.y, win.w, win.h, menu.theme.outline)

    -- Title bar
    valex.draw_filled_rect(win.x, win.y, win.w, 30, menu.theme.title_bar)
    local title_w, title_h = valex.get_text_size(win.title)
    valex.draw_text(win.title, win.x + (win.w / 2) - (title_w / 2), win.y + 15 - (title_h / 2), menu.theme.text)

    -- Tabs
    local tab_x = win.x + 5
    for i, tab in ipairs(win.tabs) do
        local color = (win.active_tab_index == i) and menu.theme.tab_active or menu.theme.tab_inactive
        valex.draw_filled_rect(tab_x, win.y + 35, tab.w, tab.h, color)
        local tab_text_w, tab_text_h = valex.get_text_size(tab.title)
        valex.draw_text(tab.title, tab_x + (tab.w / 2) - (tab_text_w / 2), win.y + 35 + (tab.h / 2) - (tab_text_h / 2), menu.theme.text)
        tab_x = tab_x + tab.w + 5
    end
...
function Button:new(props)
    ...
    obj.x = props.x or 10; obj.y = props.y or 10
    obj.w = props.w or 100; obj.h = props.h or 25
...
function Button:render(win_x, win_y, is_selected)
    ...
    valex.draw_filled_rect(abs_x, abs_y, self.w, self.h, menu.theme.control_background)
    if is_selected then
        valex.draw_rect(abs_x, abs_y, self.w, self.h, menu.theme.control_selected, 2.0)
    end
    local text_w, text_h = valex.get_text_size(self.title)
    valex.draw_text(self.title, abs_x + (self.w / 2) - (text_w / 2), abs_y + (self.h / 2) - (text_h / 2), menu.theme.text)
end
...
function Toggle:new(props)
    ...
    obj.callback = props.callback or function() print(obj.title .. " toggled to " .. tostring(obj.toggled)) end
    return obj
end
...
function Toggle:render(win_x, win_y, is_selected)
    ...
    if self.toggled then
        valex.draw_filled_rect(abs_x + 3, abs_y + 3, self.w - 6, self.h - 6, menu.theme.control_selected)
    end
    local _, text_h = valex.get_text_size(self.title)
    valex.draw_text(self.title, abs_x + self.w + 10, abs_y + (self.h / 2) - (text_h / 2), menu.theme.text)
end
...
function menu:CreateWindow(props)
    local new_window = {
        ...
        w = props.width or 300, h = props.height or 400,
        x = props.x or 100, y = props.y or 100,
        visible = true,
        tabs = {},
        active_tab_index = 1
    }
...
function menu:AddTab(parent_window, props)
    local new_tab = {
        ...
        w = props.width or 80, h = props.height or 25,
        controls = {},
